#!/usr/bin/env python3
"""
Minimal UMAP visualization v3 - customizable thresholds, 5 colors, translucent
"""
...
print("=== Creating Minimal UMAP Visualization v3 ===")
...
# Calculate statistics for default thresholds
ai_p10 = essays_df['ai_rating'].quantile(0.10)
ai_p90 = essays_df['ai_rating'].quantile(0.90)

print(f"AI Rating range: {essays_df['ai_rating'].min():.2f} - {essays_df['ai_rating'].max():.2f}")
print(f"AI Rating percentiles: P10={ai_p10:.2f}, P90={ai_p90:.2f}")
...
<html>
<head>
    <title>UMAP Visualization - Customizable Thresholds</title>
...
<body>
    <div id="info">
        ...
        <div class="control-group">
            ...
            <div>
                Low: &lt; <input type="number" id="ai-low" class="threshold-input" value=\"""" + f"{ai_p10:.1f}" + """\" min="1" max="7" step="0.1">
                High: &gt; <input type="number" id="ai-high" class="threshold-input" value=\"""" + f"{ai_p90:.1f}" + """\" min="1" max="7" step="0.1">
            </div>
        </div>
        ...
        <div class="control-group">
            ...
            <div>
                Low: ≤ <input type="number" id="sc-low" class="threshold-input" value="2" min="1" max="5" step="1">
                High: ≥ <input type="number" id="sc-high" class="threshold-input" value="4" min="1" max="5" step="1">
            </div>
        </div>
        ...
        <div style="margin-top: 15px;">
            ...
            <div class="legend-item">
                <span class="color-box" style="background: rgba(68,68,255,0.8);"></span>
            ...
            <div class="legend-item">
                <span class="color-box" style="background: rgba(255,255,68,0.8);"></span>
    ...
    <script>
        ...
        // Colors for categories (with alpha for translucency)
        const categoryColors = {
            'both_high': [1.0, 0.27, 0.27],    // Red
            'ai_high': [0.27, 0.27, 1.0],      // Blue
            'sc_high': [0.27, 1.0, 0.27],      // Green
            'both_low': [1.0, 1.0, 0.27],      // Yellow
            'middle': [0.53, 0.53, 0.53]       // Gray
        };
        
        function updateCategories() {
            const aiLow = parseFloat(document.getElementById('ai-low').value);
            const aiHigh = parseFloat(document.getElementById('ai-high').value);
            const scLow = parseInt(document.getElementById('sc-low').value);
            const scHigh = parseInt(document.getElementById('sc-high').value);
...
# Save
output_file = OUTPUT_DIR / 'minimal_umap_viz_v3.html'
with open(output_file, 'w') as f:
    f.write(html_content)

print(f"\n✅ Customizable visualization saved to: {output_file}")
print("\nFeatures:")
print("- Customizable thresholds for AI rating and social class")
print("- 5 colors: 4 extremes + gray for middle")
print("- Translucent points (adjustable opacity)")
print("- Essay display at bottom with matching background color")
print("- Rotation centered on data cloud, not origin")
print("- Real-time category counts")