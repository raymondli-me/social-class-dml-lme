...
GPU-ACCELERATED VERSION for XGBoost and Random Forest
"""
...
# Try to import CuML for GPU-accelerated Random Forest
try:
    from cuml.ensemble import RandomForestRegressor as cuRF
    CUML_AVAILABLE = True
    print("✓ CuML available - will use GPU-accelerated Random Forest")
except ImportError:
    CUML_AVAILABLE = False
    print("⚠ CuML not available - will use CPU Random Forest")
...
# Add GPU-accelerated Random Forest if CuML is available
if CUML_AVAILABLE:
    models['RandomForest'] = {
        'model_y': cuRF(n_estimators=100, max_depth=10, random_state=42),
        'model_t': cuRF(n_estimators=100, max_depth=10, random_state=42)
    }
else:
    models['RandomForest'] = {
...
def run_analysis(X, X_name, results_key):
    ...
    for model_name, model_config in models.items():
        ...
        # For GPU models, we'll do manual cross-validation to avoid sklearn's overhead
        if CUML_AVAILABLE and model_name == 'RandomForest':
            # Manual CV for CuML
            ...
            for train_idx, test_idx in kf.split(X):
                ...
                # AI prediction
                model_ai = cuRF(n_estimators=100, max_depth=10, random_state=42)
                model_ai.fit(X_train, y_train)
                y_pred = model_ai.predict(X_test)
                ai_r2 = 1 - np.sum((y_test - y_pred)**2) / np.sum((y_test - y_test.mean())**2)
                ai_scores.append(ai_r2)
                
                # SC prediction
                model_sc = cuRF(n_estimators=100, max_depth=10, random_state=42)
                model_sc.fit(X_train, d_train)
                d_pred = model_sc.predict(X_test)
                sc_r2 = 1 - np.sum((d_test - d_pred)**2) / np.sum((d_test - d_test.mean())**2)
                sc_scores.append(sc_r2)
            
            ai_r2 = np.mean(ai_scores)
            ai_r2_std = np.std(ai_scores)
            sc_r2 = np.mean(sc_scores)
            sc_r2_std = np.std(sc_scores)
        else:
            # Use sklearn's cross_val_score for other models
...
for model_name in models.keys():
    ...
    gpu_marker = " [GPU]" if model_name in ['RandomForest', 'XGBoost'] else ""
    print(f"{model_name:<12}{gpu_marker} | {full_ai:<10.4f} {full_sc:<10.4f} {full_time:<8.1f} | "
...
for model_name in models.keys():
    ...
    gpu_marker = " [GPU]" if model_name in ['RandomForest', 'XGBoost'] else ""
    print(f"{model_name:<12}{gpu_marker} | {full_theta:<10.6f} {full_se:<10.6f} {full_p:<10.4f} | "
...
gpu_models = ['RandomForest', 'XGBoost']
if any(m in models for m in gpu_models):
    print(f"\n3. GPU ACCELERATION IMPACT:")
    for model in gpu_models:
        if model in results['full_dims']['r2']:
            full_time = results['full_dims']['r2'][model]['time']
            pca_time = results['pca_200']['r2'][model]['time']
            print(f"   {model}: Full dims {full_time:.1f}s, PCA {pca_time:.1f}s")

print(f"\n4. COMPUTATIONAL EFFICIENCY:")
print(f"   Full dimensions: {avg_time_full:.1f}s average")
print(f"   PCA dimensions: {avg_time_pca:.1f}s average")
print(f"   Full dims are {time_ratio:.1f}x slower overall")

# Save results
output_file = NVEMBED_DIR / 'nvembed_human_macarthur_gpu_comparison.pkl'
...
if abs(avg_ai_imp) < 0.02 and abs(avg_sc_imp) < 0.02:
    ...
    print("- GPU acceleration available for XGBoost/RF helps with speed")
