#!/usr/bin/env python3
"""
Generate embeddings using Google's Gemma-2b model for embeddings.
Since Gemma-Embeddings-v1.0 has loading issues, we'll use gemma-2b which can produce embeddings.
"""
...
from transformers import AutoTokenizer, AutoModel, GemmaModel
import torch
...
class GemmaEmbedder:
    """Generate embeddings using Gemma-2b model"""
    ...
    def __init__(self, model_name: str = "google/gemma-2b", device: str = None):
        ...
        # Load Gemma model specifically for embeddings
...
def generate_embeddings(df: pd.DataFrame, cache_file: Path = None) -> np.ndarray:
    ...
    # Initialize embedder
    embedder = GemmaEmbedder(model_name="google/gemma-2b")
...
def main():
    ...
    print("Using google/gemma-2b model")
    print("="*60)
    ...
    # Generate embeddings
    embeddings_file = CHECKPOINT_DIR / "gemma_2b_embeddings.npy"
    ...
    # Save PCA features
    pca_file = CHECKPOINT_DIR / "gemma_2b_pca_200_features.pkl"
    ...
    results_file = CHECKPOINT_DIR / "gemma_2b_analysis_results.pkl"
