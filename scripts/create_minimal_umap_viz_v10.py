#!/usr/bin/env python3
"""
Minimal UMAP visualization v10 - Simplified TreeSHAP with better readability
"""
...
print("=== Creating Minimal UMAP Visualization v10 ===")
...
# Calculate global PC effects
print("Calculating global PC effects...")
pc_global_effects = {}
for pc_idx in range(200):
    # Create test data: high vs low PC values
    test_data = np.zeros((2, 200))
    test_data[0, pc_idx] = np.percentile(X_pca[:, pc_idx], 90)  # High PC value (90th percentile)
    test_data[1, pc_idx] = np.percentile(X_pca[:, pc_idx], 10)  # Low PC value (10th percentile)
    ...
    if not pd.isna(essays_df.iloc[i]['sc11']) and not pd.isna(essays_df.iloc[i]['ai_rating']):
        ...
        for pc_idx in top_10_indices:
            ...
            pc_info.append({
                ...
                'variance_ai': float(np.abs(contributions_ai[i, pc_idx]) * variance_explained[pc_num] * 100),
                'variance_sc': float(np.abs(contributions_sc[i, pc_idx]) * variance_explained[pc_num] * 100)
            })
...
<html>
<head>
    <title>UMAP Visualization - TreeSHAP Analysis</title>
...
<body>
    ...
    <div id="pc-global-info" class="pc-global-info">
        <h4 id="pc-title"></h4>
    ...
    <script>
        ...
        function showPCGlobalInfo() {
            ...
            const effects = pcGlobalEffects[pcIndex];
            document.getElementById('pc-title').textContent = `PC${pcIndex} Global Effects`;
            
            const content = `
                <table class="prob-table">
                    <tr>
                        <td>If HIGH in PC${pcIndex} (90th percentile):</td>
                        <td></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>If LOW in PC${pcIndex} (10th percentile):</td>
                        <td></td>
                    </tr>
                    <tr>
                        <td style="padding-left: 20px;">→ High AI Rating</td>
...
# Save
output_file = OUTPUT_DIR / 'minimal_umap_viz_v10.html'
with open(output_file, 'w') as f:
    f.write(html_content)

print(f"\n✅ Simplified TreeSHAP visualization saved to: {output_file}")
print("\nChanges in v10:")
print("- Removed point size modifications for PC extremes")
print("- Removed PC highlighting feature")
print("- Changed variance explained to show AI and SC separately")
print("- Removed white borders (not implemented)")
print("- Increased essay background opacity range (0.5-1.0, default 0.9)")
print("- Added black text outlines for better readability")
print("- Added global PC effects dropdown showing probabilities")
print("- Simplified interface while maintaining core functionality")