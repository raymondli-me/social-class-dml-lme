#!/usr/bin/env python3
"""
Minimal UMAP visualization v5 - black background, distinct colors, percentages
"""
...
print("=== Creating Minimal UMAP Visualization v5 ===")
...
<html>
<head>
    <title>UMAP Visualization - Dark Theme</title>
...
<body>
    ...
    <script>
        ...
        // Lights - adjusted for dark background
        ...
        // Point material - brighter for dark background
        ...
        // Highly distinct colors for dark background
        ...
        function updateCategories() {
            ...
            // Update counts display with percentages
            const total = data.length;
            document.getElementById('counts').innerHTML = `
                <strong>Counts:</strong><br>
                High AI + High SC: ${counts.both_high} (${(counts.both_high/total*100).toFixed(1)}%)<br>
                High AI + Low SC: ${counts.ai_high} (${(counts.ai_high/total*100).toFixed(1)}%)<br>
                Low AI + High SC: ${counts.sc_high} (${(counts.sc_high/total*100).toFixed(1)}%)<br>
                Low AI + Low SC: ${counts.both_low} (${(counts.both_low/total*100).toFixed(1)}%)<br>
        ...
        function onMouseMove(event) {
            ...
            if (intersects.length > 0) {
                ...
                if (newIndex !== hoveredIndex) {
                    ...
                    // Set border color based on point color
                    const color = categoryColors[category];
                    const borderColor = `rgb(${Math.floor(color[0]*255)}, ${Math.floor(color[1]*255)}, ${Math.floor(color[2]*255)})`;
                    document.getElementById('essay-display').style.borderColor = borderColor;
...
# Save
output_file = OUTPUT_DIR / 'minimal_umap_viz_v5.html'
with open(output_file, 'w') as f:
    f.write(html_content)

print(f"\n✅ Dark theme visualization saved to: {output_file}")
print("\nFeatures:")
print("- Black background with bright, distinct colors")
print("- Colors: Green=high both, Magenta=high AI, Cyan=high SC, Yellow=low both")
print("- Counts now show both numbers and percentages")
print("- Essay display has colored border matching point category")
print("- Adjusted lighting and opacity for dark theme")