#!/usr/bin/env python3
"""
DML analysis with UMAP visualization using THREE.InstancedMesh for precise hovering
Built from ground up with reliable hover detection
"""
...
print("=== InstancedMesh DML PC Analysis ===")
...
with open(OUTPUT_DIR / 'dml_pc_analysis_results_with_umap.pkl', 'rb') as f:
    ...
    X_umap_3d = full_results['umap_3d']
...
# Top 5 PCs (using full results)
top_pcs = full_results['top_pcs'][:5]
W = X_pca[:, top_pcs]
...
# Get predictions and effects from stored results
Y_pred_cv = full_results['predictions']['y_pred_full']
T_pred_cv = full_results['predictions']['t_pred_full']
treatment_effects = full_results['predictions']['treatment_effects_full']

# Prepare visualization data
viz_data = []
for i in range(len(essays_valid)):
    pc_data = {}
    for j, pc_idx in enumerate(top_pcs):
        pc_num = pc_idx + 1  # 1-based for display
        pc_val = W[i, j]
        percentile = stats.percentileofscore(W[:, j], pc_val)
        pc_data[f'pc{pc_num}_value'] = float(pc_val)
        pc_data[f'pc{pc_num}_percentile'] = float(percentile)
...
# DML summary
dml_summary = full_results['dml_results']['full']
