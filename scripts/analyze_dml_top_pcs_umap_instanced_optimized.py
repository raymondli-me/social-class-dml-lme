#!/usr/bin/env python3
"""
Optimized InstancedMesh DML analysis - fixes performance issues
"""
...
print("=== Optimized InstancedMesh DML Analysis ===")
...
print(f"\n3. Creating optimized InstancedMesh visualization...")

# HTML template with optimized InstancedMesh
html_template = """
<!DOCTYPE html>
<html>
<head>
    <title>DML Analysis - Optimized InstancedMesh</title>
...
<body>
    ...
    <script>
        ...
        // Create base geometry - VERY LOW POLY sphere
        const baseGeometry = new THREE.SphereGeometry(0.5, 6, 4);  // Even lower poly!
        
        // Create material - SIMPLE
        ...
        // Pre-calculate color ranges
        const colorRanges = {{
            ai_rating: {{
                min: Math.min(...data.map(d => d.ai_rating)),
                max: Math.max(...data.map(d => d.ai_rating))
            }}
        }};
        
        // Initialize instances - ONE TIME
        function initializeInstances() {{
            console.time('Initialize Instances');
            
            data.forEach((d, i) => {{
                // Set position and scale
                tempMatrix.makeScale(currentPointSize, currentPointSize, currentPointSize);
                tempMatrix.setPosition(d.x * currentScaleFactor, d.y * currentScaleFactor, d.z * currentScaleFactor);
            ...
            console.timeEnd('Initialize Instances');
        }}
        ...
        function updateInstanceColor(index) {{
            ...
            if (currentColorMode === 'social_class') {{
                ...
                rgb = getGradientColor(d.ai_rating, colorRanges.ai_rating.min, colorRanges.ai_rating.max);
        ...
        // Update all colors - ONLY when color mode changes
        function updateColors() {{
            console.time('Update Colors');
        ...
        // Raycaster for precise hovering - with throttling
        ...
        function updateHover() {{
            if (!mouseNeedsUpdate) return;
            mouseNeedsUpdate = false;
            
            mouse.x = (lastMouseX / window.innerWidth) * 2 - 1;
            mouse.y = -(lastMouseY / window.innerHeight) * 2 + 1;
            ...
            if (intersects.length > 0) {{
                ...
                if (visibilityArray[instanceId] && instanceId !== hoveredInstanceId) {{
                    ...
                    if (rect.right > window.innerWidth) {{
                        tooltip.style.left = (lastMouseX - rect.width - 15) + 'px';
                    }}
                    if (rect.bottom > window.innerHeight) {{
                        tooltip.style.top = (lastMouseY - rect.height - 15) + 'px';
                    }}
                }}
            }} else {{
                hoveredInstanceId = -1;
        ...
        // Update visibility - ONLY when filters/size/scale change
        function updateVisibility() {{
            console.time('Update Visibility');
            let visibleCount = 0;
            ...
            console.timeEnd('Update Visibility');
        }}
        ...
        // Display controls - use 'change' event for less frequent updates
        ...
        // Event listeners - use 'change' for sliders to reduce updates
        document.getElementById('point-size').addEventListener('change', updatePointSize);
        document.getElementById('point-size').addEventListener('input', (e) => {{
            document.getElementById('size-val').textContent = parseFloat(e.target.value).toFixed(1);
        }});
        
        document.getElementById('point-opacity').addEventListener('change', updateOpacity);
        ...
        document.getElementById('cloud-scale').addEventListener('change', updateCloudScale);
        document.getElementById('cloud-scale').addEventListener('input', (e) => {{
        ...
        for (let pc = 1; pc <= 5; pc++) {{
            ...
            document.getElementById(`pc${{pc}}-min`).addEventListener('change', () => {{
                if (pcFilters[`pc${{pc}}`]) {{
                    pcFilters[`pc${{pc}}`].min = parseFloat(document.getElementById(`pc${{pc}}-min`).value);
                    updateVisibility();
                }}
            }});
            document.getElementById(`pc${{pc}}-max`).addEventListener('change', () => {{
                if (pcFilters[`pc${{pc}}`]) {{
        ...
        // Animation loop - MINIMAL work here
        function animate() {{
            requestAnimationFrame(animate);
            
            // Update hover only once per frame
            updateHover();
        ...
        console.log('InstancedMesh Optimization Summary:');
        console.log('- Lower poly sphere (6x4 segments)');
        console.log('- Reused Matrix4/Color objects');
        console.log('- Updates only on control changes');
        console.log('- Throttled hover with requestAnimationFrame');
        console.log('- Pre-calculated color ranges');
    </script>
</body>
</html>
"""
...
# Save HTML
output_file = OUTPUT_DIR / 'umap_dml_instanced_optimized.html'
with open(output_file, 'w') as f:
    f.write(html_content)

print(f"\n✅ Optimized InstancedMesh visualization saved to: {output_file}")
print("\nOptimizations:")
print("- Lower poly spheres (6x4 segments vs 8x6)")
print("- Reused Matrix4 and Color objects (no garbage collection)")
print("- Updates only on control changes (not every frame)")
print("- Throttled hover detection with requestAnimationFrame")
print("- 'change' events for sliders instead of 'input'")
print("- Pre-calculated color ranges")
print("- Console timing for performance monitoring")